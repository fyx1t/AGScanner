...
class Fuzzer:
    ...
    def work(self):
        ...
        for fuzzer in [fuzzer for endpoint in self.pool.keys() for fuzzer in self.pool[endpoint] if fuzzer['name']]:
            if fuzzer['name'] not in self.usable_fuzzers.keys():
                ...
                self.usable_fuzzers[fuzzer['name']] = {
                    'configs': load(open(
                        f"{
                            self.fuzzers['all'][fuzzer['claster']][fuzzer['name']]['path']}/{
                            self.fuzzers['all'][fuzzer['claster']][fuzzer['name']]['configs']}"
                        )), 
                    'data': fuzzer, 
                    'path': f"{self.fuzzers['all'][fuzzer['claster']][fuzzer['name']]['path']}"}
        ...
        for fuzzer in self.usable_fuzzers.keys():
            self.results.append(
                BaseFuzzer(
                        self.usable_fuzzers[fuzzer]['configs']['payloads_collection'], 
                        self.usable_fuzzers[fuzzer]['path']).fuzz(self.usable_fuzzers[fuzzer]['data'])
                    )


class BaseFuzzer:
    def __init__(self, payloads_collections: list, location: str):
        super().__init__()
