from modules.alanyzer.checker import Checker
...
class Analyzer:
    """
    Класс, ответственный за проведение этапа анализа каждого ресурса тестируемого сервиса.
    Класс позволяет получить перечень всех ресурсов, полученных на предыдущем этапе, и для 
    каждого ресурса, за счет взаимодействия с интерфейсом Checker, получить информацию
    о том, какими фаззерами его тестировать.
    """
    def __init__(self):
    ...
    def __identify_fuzzers_pool(self, endpoint: str) -> list:
        ...
        accepted_rules = self.checker.run(endpoint)
    ...
    def identify_entry_points(self):
        ...
        print("[MESSAGE] - POOL UPDATED WITH: ", self.pool)
